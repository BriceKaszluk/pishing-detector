...
const Section: React.FC<SectionProps> = ({ title, explanation1, explanation2, videoSrc }) => {
  return (
    <Grid container direction={{ sm: 'column-reverse', md: 'row' }} className="md:min-h-screen mb-8 md:mb-0">
      <Grid item md={7} className="text-gray-800 flex justify-center items-center w-full mt-8 px-2" order={{ xs: 2, md: 1 }}>
        <video src={videoSrc} autoPlay muted loop className='rounded-lg border-2 shadow-lg'></video>
      </Grid>
      <Grid item md={5} className="text-gray-800 flex flex-col justify-center mt-8 px-2 lg:px-40" order={{ xs: 1, md: 2 }}>
